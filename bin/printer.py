    ...
    for file_name in file_list:
        ...
        for j in range(copies):
            try:
                ...
                print("调用相关文档程序失败，请将福昕阅读器作为打开PDF的默认程序，并保证您的计算机的默认打印机"
                      "不是类似于pdf打印机的这种文件打印机")
            else:
                print('正在打印第' + str(printing_copies) + "份" + file_name)
                printing_copies = printing_copies + 1
def print_local_task(file_path):
    local_files_list=os.listdir(file_path)
    for file_name in local_files_list:
        ...
        if re.match(r'.*.((pdf)|(doc)|(docx))$', file_name):
            try:
                ...
                print("调用相关文档程序失败，请将福昕阅读器作为打开PDF的默认程序、安装office软件。并保证您的"
